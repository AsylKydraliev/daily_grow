...
const form = useForm({
    branch_id: props.debt.branch_id,
    name: props.debt.name,
    amount: props.debt.amount,
    debt_date: props.debt.debt_date,
})
...
<template>
    <MainLayout>
        <h3 class="text-xl font-bold mb-5">Редактировать долг</h3>
        <form class="sm:max-w-lg" @submit.prevent="handleSubmit">
            ...
            <div class="mb-5">
                <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Название долга</label>
                <input
                    type="text"
                    id="name"
                    v-model="form.name"
            ...
            <div class="mb-5">
                <label for="amount" class="block mb-2 text-sm font-medium text-gray-900">Сумма</label>
                <input
                    type="number"
                    step="0.01"
                    id="amount"
            ...
            <div class="mb-5">
                <label for="debt_date" class="block mb-2 text-sm font-medium text-gray-900">Дата долга</label>
                <input
                    type="date"
                    id="debt_date"
                    v-model="form.debt_date"
                    :class="{ 'border-red-500': form.errors.debt_date }"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                />
                <small v-if="form.errors.debt_date" class="text-red-600">{{ form.errors.debt_date }}</small>
            </div>
